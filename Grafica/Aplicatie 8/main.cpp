...
int texUniform, matrixLocation, useTextureLocation;
...
{
    int uniformID = glGetUniformLocation(shader, uniform.c_str());

    if (uniformID == -1)
        std::cerr << "Error when getting uniform location.\n";
...
void initUniforms()
{
    ...
    matrixLocation = getUniformLocation("u_MVP");
